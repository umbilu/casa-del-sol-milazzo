...
const AIConcierge: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: 'model', text: 'Ciao! Sono il tuo concierge virtuale. Vuoi sapere qualcosa su Casa del Sol o sulle gite alle Isole Eolie?' }
  ]);
    ...
    <div className="fixed bottom-6 right-6 z-[60]">
      ...
      {!isOpen && (
        <button
          ...
          className="w-16 h-16 bg-orange-600 rounded-full shadow-2xl flex items-center justify-center text-white text-2xl hover:scale-110 transition-transform animate-bounce hover:animate-none border-4 border-white"
        >
        ...
        <div className="w-80 md:w-96 h-[500px] bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-slate-200 animate-fade-in-up">
          ...
          <div className="p-4 border-t border-slate-100 bg-white flex space-x-2">
            <input
              ...
              className="flex-1 bg-slate-100 rounded-full px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-600"
            />
            <button
              onClick={handleSend}
